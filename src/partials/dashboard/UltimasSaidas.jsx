...
export function UltimasSaidas() {
  const { despesas } = useContext(userContext);
  return (
    <div className="col-span-full bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      <header className="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
        <h2 className="font-semibold text-slate-800 dark:text-slate-100">Ultimas Saidas</h2>
      </header>
      <div className="p-3">
        {/* Table */}
            ...
            <tbody className="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
              {/* Row */}
              {
                despesas.map((despesa) => {
                  ...
                  const [ano, mes, dia] = String(despesa.data).split("-");
                    ...
                    <tr>
                      <td className="p-2">
                        <div className="flex items-center">
                          <div className="text-slate-800 dark:text-slate-100">{despesa.descricao}</div>
                        </div>
                      </td>
                      <td className="p-2">
                        <div className="text-center">{despesa.pagamento}</div>
                      </td>
                      <td className="p-2">
                        <div className="text-center text-red-500">R$ {
